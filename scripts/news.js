...
// TASK 7: Chuyển trang cho các bài viết
...
const appendPageNumbers = function (index) {
  ...
  // document
  //   .getElementById("list-next-btn")
  //   .insertAdjacentElement("beforebegin", pageNum);
};

let settings = getFromStorage("settings");

const getNews = async function (
  category = settings.categorySettings ?? "general",
  page = 1,
  pageSize = settings.pageSizeSettings ?? 5
) {
  let response = await fetch(
    `https://newsapi.org/v2/top-headlines?country=us&category=${category}&pageSize=${pageSize}&page=${page}&apiKey=b0035e1c316f4c0cb3be06bb843d8656`
  );
